...
export const execute = async (params: DandisetFilesParams): Promise<string> => {
  try {
    ...
    if (!response1.ok) {
      throw new Error(
        `DANDI API error: ${response1.status} ${response1.statusText}`,
      );
    }
    ...
    if (!response2.ok) {
      throw new Error(
        `DANDI API error: ${response2.status} ${response2.statusText}`,
      );
    }

    const data2 = await response2.json();

    const files: DandiAsset[] = data2.results.map(
      (asset: DandiAssetResponse) => ({
        path: asset.path,
        size: asset.size,
