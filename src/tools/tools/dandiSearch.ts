...
export const toolFunction: ORFunctionDescription = {
  ...
  parameters: {
    ...
    properties: {
      ...
      includeEmbargoed: {
        type: 'boolean',
        description: 'Whether to include embargoed dandisets in search results (requires API key)',
        default: false
      }
    }
  }
...
export const execute = async (params: DandiSearchParams): Promise<string> => {
  ...
  try {
    ...
    const formattedResults: DandisetBasicInfo[] = results.map(result => {
      const version = result.most_recent_published_version || result.draft_version
      return {
