...
export const toolFunction: ORFunctionDescription = {
  ...
  description:
    "Search for dandisets on DANDI archive using semantic search (finds similar dandisets based on description)",
  parameters: {
    type: "object",
    properties: {
      query: {
...
export const execute = async (
  params: DandiSemanticSearchParams,
): Promise<string> => {
  ...
  try {
    ...
    if (!semanticResponse.ok) {
      throw new Error(
        `Semantic search API error: ${semanticResponse.status} ${semanticResponse.statusText}`,
      );
    }
    ...
    const detailsPromises = limitedIds.map(async (dandisetId) => {
      try {
        ...
        if (!response.ok) {
          console.error(
            `Error fetching details for dandiset ${dandisetId}: ${response.status}`,
          );
          return null;
        }
        ...
        const version =
          data.most_recent_published_version || data.draft_version;
        ...
        console.error(
          `Error fetching details for dandiset ${dandisetId}:`,
          error,
        );
        return null;
      }
    ...
    const validDetails = details.filter(
      (d): d is DandisetBasicInfo => d !== null,
    );
